struct Distance {
  ...
  double transformed_distance(const Vertex& p1, const Vertex& p2) const {
  ...
  double min_distance_to_rectangle(const Vertex& p,
  ...
  template <class TreeTraits>
  double max_distance_to_rectangle(const Vertex& p,
                                   const CGAL::Kd_tree_rectangle<TreeTraits>& b) const {
    double h = p.x();
